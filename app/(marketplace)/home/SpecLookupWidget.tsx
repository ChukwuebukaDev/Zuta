...
// import { Sliders, Activity, Gauge, Disc, HelpCircle, Loader2 } from "lucide-react";
...
//   const [displaySpecs, setDisplaySpecs] = useState<{ [key: string]: string } | null>(null);
//   const [loading, setLoading] = useState(false);

//   // Generate Year Array (Current year down to 2000)
//   const currentYear = new Date().getFullYear();
//   const yearsList = Array.from({ length: currentYear - 1999 }, (_, i) => String(currentYear - i));

//   // 🚀 FIXED: Hoisted standard function syntax completely bypasses the Temporal Dead Zone (TDZ)
//   function compileFallbackMetadata(brand: string, model: string, year: string) {
...
//   // 🔄 Watcher Hook Loop sits safely under hoisted helper utilities
//   useEffect(() => {
//     if (!selectedBrand || !selectedModel || !selectedYear) {
//       setTimeout(() => setDisplaySpecs(null), 0);
//       return;
//     }

//     const fetchLiveSpecifications = async () => {
//       setLoading(true);
//       try {
...
//         const response = await fetch(`/api/car-specs/details?${queryParams}`);
//         if (!response.ok) throw new Error("Proxy error");
        
//         const data = await response.json();
//         console.log("Live Spec Data:", data);
//         setDisplaySpecs(data);
//       } catch (err) {
//         console.error("Failed to fetch from live registry:", err);
//         // Safely targets hoisted configuration mapping arrays with zero errors
//         compileFallbackMetadata(selectedBrand, selectedModel, selectedYear);
//       } finally {
//         setLoading(false);
//       }
//     };

//     fetchLiveSpecifications();
...
//       <div className="absolute -top-24 -left-24 w-48 h-48 bg-blue-500/10 blur-[100px] rounded-full animate-none" />
//       <div className="absolute -bottom-24 -right-24 w-48 h-48 bg-indigo-500/5 blur-[100px] rounded-full animate-none" />
...
//       {/* Dropdown Selection Inputs Matrix Row */}
...
//       {/* Spec Output Results Sheets / Loader Frames */}
//       {loading ? (
//         <div className="mt-6 p-12 bg-zinc-950/20 border border-slate-900 rounded-2xl flex flex-col items-center justify-center text-center relative z-10">
//           <Loader2 size={24} className="text-blue-500 animate-spin" />
//           <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-3">Compiling live vehicle specifications...</p>
//         </div>
//       ) : displaySpecs ? (
...
//               <p className="text-xs font-bold text-slate-200 truncate mt-0.5 capitalize">{displaySpecs.bodyType?.toLowerCase()}</p>
//             </div>
//           </div>
...
//               <p className="text-xs font-bold text-slate-300 truncate mt-0.5 uppercase">{displaySpecs.driveType?.split(" ")[0]} Layout</p>
//             </div>
//           </div>

//         </div>
//       ) : (
//         <div className="mt-4 p-6 bg-zinc-950/20 border border-dashed border-slate-900/80 rounded-2xl flex flex-col items-center justify-center text-center group relative z-10">
